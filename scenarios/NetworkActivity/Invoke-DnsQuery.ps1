...
<#
.SYNOPSIS
DnsQuery 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Network Activity
场景 ID：win.network.dns_query
默认威胁等级：LV1
需要管理员：false
行为类型：network.dns_query
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-DnsQuery.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.network.dns_query'
