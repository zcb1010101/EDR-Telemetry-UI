...
<#
.SYNOPSIS
TcpConnection 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Network Activity
场景 ID：win.network.tcp
默认威胁等级：LV1
需要管理员：false
行为类型：network.tcp
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-TcpConnection.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.network.tcp'
