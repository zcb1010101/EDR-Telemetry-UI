...
<#
.SYNOPSIS
UdpConnection 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Network Activity
场景 ID：win.network.udp
默认威胁等级：LV1
需要管理员：false
行为类型：network.udp
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-UdpConnection.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.network.udp'
