...
<#
.SYNOPSIS
FileDownload 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Network Activity
场景 ID：win.network.file_download
默认威胁等级：LV2
需要管理员：false
行为类型：network.file_download
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-FileDownload.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.network.file_download'
