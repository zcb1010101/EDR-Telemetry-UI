...
<#
.SYNOPSIS
Md5HashTxt 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Hash Algorithms
场景 ID：win.hash.md5.txt
默认威胁等级：LV0
需要管理员：false
行为类型：hash.md5
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-Md5HashTxt.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.hash.md5.txt'
