...
<#
.SYNOPSIS
ShaHashTxt 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Hash Algorithms
场景 ID：win.hash.sha.txt
默认威胁等级：LV0
需要管理员：false
行为类型：hash.sha
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-ShaHashTxt.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.hash.sha.txt'
