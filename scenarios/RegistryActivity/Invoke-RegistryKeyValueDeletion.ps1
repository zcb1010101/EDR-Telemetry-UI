...
<#
.SYNOPSIS
RegistryKeyValueDeletion 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Registry Activity
场景 ID：win.registry.delete
默认威胁等级：LV1
需要管理员：false
行为类型：registry.delete
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-RegistryKeyValueDeletion.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.registry.delete'
