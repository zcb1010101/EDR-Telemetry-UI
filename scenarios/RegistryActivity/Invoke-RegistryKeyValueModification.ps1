...
<#
.SYNOPSIS
RegistryKeyValueModification 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Registry Activity
场景 ID：win.registry.modify
默认威胁等级：LV1
需要管理员：false
行为类型：registry.modify
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-RegistryKeyValueModification.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.registry.modify'
