...
<#
.SYNOPSIS
GroupPolicyModification 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Other Relevant Events
场景 ID：win.group_policy.modify
默认威胁等级：LV2
需要管理员：true
行为类型：group_policy.modify
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-GroupPolicyModification.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.group_policy.modify'
