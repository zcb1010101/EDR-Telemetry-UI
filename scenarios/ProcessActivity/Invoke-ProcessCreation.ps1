...
<#
.SYNOPSIS
ProcessCreation 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Process Activity
场景 ID：win.process.create
默认威胁等级：LV0
需要管理员：false
行为类型：process.create
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-ProcessCreation.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.process.create'
