...
<#
.SYNOPSIS
ProcessAccess 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Process Activity
场景 ID：win.process.access
默认威胁等级：LV1
需要管理员：false
行为类型：process.access
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-ProcessAccess.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.process.access'
