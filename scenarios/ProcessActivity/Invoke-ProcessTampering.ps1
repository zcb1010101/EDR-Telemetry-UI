...
<#
.SYNOPSIS
ProcessTampering 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Process Activity
场景 ID：win.process.tampering
默认威胁等级：LV3
需要管理员：false
行为类型：process.tampering
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-ProcessTampering.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.process.tampering'
