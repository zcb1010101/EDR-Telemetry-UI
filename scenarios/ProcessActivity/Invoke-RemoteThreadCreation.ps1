...
<#
.SYNOPSIS
RemoteThreadCreation 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Process Activity
场景 ID：win.process.remote_thread
默认威胁等级：LV2
需要管理员：false
行为类型：process.remote_thread
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-RemoteThreadCreation.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.process.remote_thread'
