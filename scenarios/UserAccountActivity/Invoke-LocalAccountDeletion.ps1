...
<#
.SYNOPSIS
LocalAccountDeletion 遥测采集与真伪校验脚本
.DESCRIPTION
类别：User Account Activity
场景 ID：win.account.local_delete
默认威胁等级：LV2
需要管理员：true
行为类型：account.local_delete
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-LocalAccountDeletion.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.account.local_delete'
