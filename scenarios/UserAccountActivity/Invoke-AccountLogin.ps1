...
<#
.SYNOPSIS
AccountLogin 遥测采集与真伪校验脚本
.DESCRIPTION
类别：User Account Activity
场景 ID：win.account.login
默认威胁等级：LV1
需要管理员：true
行为类型：account.login
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-AccountLogin.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.account.login'
