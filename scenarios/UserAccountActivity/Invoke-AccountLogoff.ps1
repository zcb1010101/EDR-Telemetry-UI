...
<#
.SYNOPSIS
AccountLogoff 遥测采集与真伪校验脚本
.DESCRIPTION
类别：User Account Activity
场景 ID：win.account.logoff
默认威胁等级：LV1
需要管理员：true
行为类型：account.logoff
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-AccountLogoff.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.account.logoff'
