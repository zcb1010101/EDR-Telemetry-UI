...
<#
.SYNOPSIS
WmiEventFilter 遥测采集与真伪校验脚本
.DESCRIPTION
类别：WMI Activity
场景 ID：win.wmi.event_filter
默认威胁等级：LV3
需要管理员：true
行为类型：wmi.event_filter
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-WmiEventFilter.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.wmi.event_filter'
