...
<#
.SYNOPSIS
WmiConsumerFilterBinding 遥测采集与真伪校验脚本
.DESCRIPTION
类别：WMI Activity
场景 ID：win.wmi.consumer_filter_binding
默认威胁等级：LV3
需要管理员：true
行为类型：wmi.consumer_filter_binding
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-WmiConsumerFilterBinding.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.wmi.consumer_filter_binding'
