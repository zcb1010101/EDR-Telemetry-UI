...
<#
.SYNOPSIS
PowerShellScriptBlock 遥测采集与真伪校验脚本
.DESCRIPTION
类别：PowerShell Activity
场景 ID：win.powershell.script_block
默认威胁等级：LV1
需要管理员：false
行为类型：powershell.script_block
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-PowerShellScriptBlock.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.powershell.script_block'
