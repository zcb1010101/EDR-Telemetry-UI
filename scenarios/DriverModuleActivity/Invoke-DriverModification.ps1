...
<#
.SYNOPSIS
DriverModification 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Driver/Module Activity
场景 ID：win.driver.modify
默认威胁等级：LV2
需要管理员：true
行为类型：driver.modify
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-DriverModification.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.driver.modify'
