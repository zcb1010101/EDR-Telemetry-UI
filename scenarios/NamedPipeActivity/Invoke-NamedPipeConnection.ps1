...
<#
.SYNOPSIS
NamedPipeConnection 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Named Pipe Activity
场景 ID：win.named_pipe.connect
默认威胁等级：LV1
需要管理员：false
行为类型：named_pipe.connect
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-NamedPipeConnection.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.named_pipe.connect'
