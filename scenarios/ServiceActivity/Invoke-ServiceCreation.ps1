...
<#
.SYNOPSIS
ServiceCreation 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Service Activity
场景 ID：win.service.create
默认威胁等级：LV2
需要管理员：true
行为类型：service.create
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-ServiceCreation.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.service.create'
