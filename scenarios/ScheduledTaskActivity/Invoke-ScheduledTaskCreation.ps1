...
<#
.SYNOPSIS
ScheduledTaskCreation 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Schedule Task Activity
场景 ID：win.scheduled_task.create
默认威胁等级：LV2
需要管理员：false
行为类型：scheduled_task.create
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-ScheduledTaskCreation.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.scheduled_task.create'
