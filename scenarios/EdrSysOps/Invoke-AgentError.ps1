...
<#
.SYNOPSIS
AgentError 遥测采集与真伪校验脚本
.DESCRIPTION
类别：EDR SysOps
场景 ID：win.edr_sysops.agent_error
默认威胁等级：LV1
需要管理员：true
行为类型：edr_sysops.agent_error
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-AgentError.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.edr_sysops.agent_error'
