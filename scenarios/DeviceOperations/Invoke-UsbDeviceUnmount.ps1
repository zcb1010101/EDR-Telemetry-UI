...
<#
.SYNOPSIS
UsbDeviceUnmount 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Device Operations
场景 ID：win.device.usb_unmount
默认威胁等级：LV0
需要管理员：false
行为类型：device.usb_unmount
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-UsbDeviceUnmount.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.device.usb_unmount'
