...
<#
.SYNOPSIS
VirtualDiskMount 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Device Operations
场景 ID：win.device.virtual_disk_mount
默认威胁等级：LV1
需要管理员：false
行为类型：device.virtual_disk_mount
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-VirtualDiskMount.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.device.virtual_disk_mount'
