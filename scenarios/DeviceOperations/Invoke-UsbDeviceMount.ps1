...
<#
.SYNOPSIS
UsbDeviceMount 遥测采集与真伪校验脚本
.DESCRIPTION
类别：Device Operations
场景 ID：win.device.usb_mount
默认威胁等级：LV0
需要管理员：false
行为类型：device.usb_mount
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-UsbDeviceMount.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.device.usb_mount'
