...
<#
.SYNOPSIS
FileOpenTxt 遥测采集与真伪校验脚本
.DESCRIPTION
类别：File Manipulation
场景 ID：win.file.open.txt
默认威胁等级：LV0
需要管理员：false
行为类型：file.open
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-FileOpenTxt.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.file.open.txt'
