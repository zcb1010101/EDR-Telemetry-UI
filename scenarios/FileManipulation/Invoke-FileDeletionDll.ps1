...
<#
.SYNOPSIS
FileDeletionDll 遥测采集与真伪校验脚本
.DESCRIPTION
类别：File Manipulation
场景 ID：win.file.delete.dll
默认威胁等级：LV1
需要管理员：false
行为类型：file.delete
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\Invoke-FileDeletionDll.ps1
#>
[CmdletBinding()]
param(
    [string]$RunId,
...
$scenario = Get-EdrScenario -ScenarioId 'win.file.delete.dll'
